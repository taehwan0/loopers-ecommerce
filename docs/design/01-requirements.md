...
- 실패 케이스
    ...
    - "판매 중" 상태가 아닌 상품이 포함된다면 주문 생성 시 에러가 발생한다.
    - 결제 정보가 유효하지 않다면 에러가 발생한다.
    - 결제에 실패한다면 에러가 발생한다. (잔액 부족, 한도 초과 등 등)
    - 결제 시스템과의 통신에 실패하면 에러가 발생한다.
    - 로그인 하지 않은 사용자에게는 오류를 반환한다.
...
| 상품 미존재       | 존재 하지 않는 상품 id가 포함됨                      | 400, Bad Request      |
| 판매중 상품 아님    | 상품의 존재가 품절, 단종 등 판매중 상태가 아님              | 409, Conflict         |
| 결제 정보 오류     | 결제 정보가 올바르지 않은 경우 (카드 번호 형식 오류, 필드 누락 등) | 400, Bad Request      |
| 결제 실패        | 결제 시스템의 결제 오류                            | 402, Payment Required |
| 결제 시스템 연동 실패 | 결제 시스템과의 통신 오류                           | 502, Bad Gateway      |
